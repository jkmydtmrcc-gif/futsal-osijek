...
function Redak({ red, polja, opisRetka, podsadrzaj, onSpremi, onObrisi }) {
  const [nacrt, setNacrt] = useState(red);
  const [stanje, setStanje] = useState('mirno');
  // Podsadržaj (npr. statistika igrača) se otvara na zahtjev — inače bi svaki
  // redak odmah povukao svoje podatke iz baze, i za one koje nitko ne otvori.
    ...
    <article className={`akartica${promijenjeno ? ' je-promijenjena' : ''}`}>
      ...
      {podsadrzaj && (
        <div className="akartica__pod">
          ...
          >
            {otvoreno ? '▾ Sakrij statistiku' : '▸ Statistika po sezonama'}
          </button>
          {otvoreno && podsadrzaj(red)}
        </div>
      )}
    </article>
  );
